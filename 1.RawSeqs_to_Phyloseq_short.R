...
### Every section labelled 1-14, was split and run in separate chunk on the High Performance Computer servers.
### Section 10-12, could be executed together without loading any workspace if all required objects were saved as .rds files.
###################################################################################################################################################
...
print( paste("Total reads: ", total.reads.3))

print( paste("Total non-chimeric reads : ", total.nonchimeric.reads.3))
...
print( paste("% Abundance of non-chimeric reads: ", nonchimera.3))
...
eqtab.nochim.3 <- readRDS("/path_to_output_directory/TMFZ/03_tabletax/seqtab.nochim.3.rds")
...
metadata <- "path_to_water_chemistry_metadata"
...
### 12. Pre-processing and selection of phyloseq for all downstream analyses ####
...
ps.edit.unassigned.reads.percent.3 <- (ps.edit.unassigned.3/sum(ps.edit.melt.3$Abundance))*100
ps.edit.chloroplast.reads.percent.3 <- (ps.edit.chloroplast.3/sum(ps.edit.melt.3$Abundance))*100
ps.edit.mitochondria.reads.percent.3 <- (ps.edit.mitochondria.3/sum(ps.edit.melt.3$Abundance))*100
...
### Sanity check : compare total number of reads
sum(sample_sums(ps.edit.3))
sum(sample_sums(ps.clean.3))
...
### 12.5. Combine data for Site labelled AF to AP and get new phyloseq
...
metadata <- as.data.frame(sapply(metadata,gsub,pattern="AF", replacement="AP")) %>% select(sample_id, Country, `GPS (decimal)`, `Human usage (Y/N)`, `Water flow (Pool/Flowing)` , Location.Code, Site, Location, Sample_id,everything())
...
metadata <- metadata %>% mutate(Region = case_when(Location.Code %in% North.Thailand ~ 'North.Thailand', Location.Code == 'PB' ~ 'Central.Thailand', Location.Code %in% South.Thailand ~ 'South.Thailand', Location.Code %in% North.Malaysia ~ 'North.Malaysia', Location.Code %in% South.Malaysia ~ 'South.Malaysia', Location.Code %in% 'SW' ~ 'Singapore'  ))
metadata <- metadata %>% select(Sample_id, Country,Region, `GPS (decimal)`, `Human usage (Y/N)`, `Water flow (Pool/Flowing)` , Location.Code, Site, Location, Sample_id,everything())
...
## Use same tax_table as clean phyloseq
...
saveRDS(new.ps.clean.3, "/path_to_output_directory/TMFZ/03_tabletax/new.ps.clean.3")
...
min.seq.depth.3 <- min(sample_sums(ps.clean.3))
max.seq.depth.3 <- max(sample_sums(ps.clean.3))
mean.seq.depth.3 <- mean(sample_sums(ps.clean.3))
median.seq.depth.3 <- median(sample_sums(ps.clean.3))
...
saveRDS(rarefied.min.3, "/path_to_output_directory/TMFZ/03_tabletax/rarefied.min.3")


### 12.7. Get Final Working Dataset (Rarefied and Filtered) = ASVs with relative abundance > 1% from rarefied dataset.
### The final working dataset is used for all analyses except for alpha diversity estimation.
...
saveRDS(rarefied.min.prop.exclude.1.3, "/path_to_output_directory/TMFZ/03_tabletax/rarefied.min.prop.exclude.1.3")
saveRDS(rarefied.min.int.exclude.1.3, "/path_to_output_directory/TMFZ/03_tabletax/rarefied.min.int.exclude.1.3")
...
### 13. Create both rooted and unrooted phylogenetic trees and add to phyloseqs
...
### Create new phyloseqs with unrooted phylogentic trees
com.rarefied.min.int.exclude.1.3 <- phyloseq(rare.ASV.wide.new,Tax.rare.wide.new,samples,phytree.rare,refseqs.rare.3)

com.rarefied.min.prop.exclude.1.3 <- phyloseq(rare.prop.wide.new,Tax.rare.wide.new,samples,phytree.rare,refseqs.rare.3)

save.image(file = "TMFZ_Renv.RData")

#############################################################################################

### 14. ### Human contaminants subsetted based on Steve's paper. #####
...
#contaminants.prop.exclude1.3 <- genus.prop.exclude.1.3 %>% subset_taxa( Genus %in% contaminants)

#rare.contaminants.prop.exclude1.3 <- rare.genus.prop.exclude.1.3 %>% subset_taxa( Genus %in% contaminants)
...
### 15. Create phyloseqs for ecological groups

#############################################################################################

## Subset for each of the 4 photosynthetic groups ####

### Chloroflexi ASVs

rarefied.chloroflexi.clean.int.exclude.1.3 <- new.rarefied.min.int.exclude.1.3 %>%
  subset_taxa(Phylum == "Chloroflexi" & Class  == "Chloroflexia")

rarefied.chloroflexi.int.wide.exclude.1.3 <- as.matrix(as.data.frame(rarefied.chloroflexi.clean.int.exclude.1.3@otu_table))

rarefied.chloroflexi.int.wide.exclude.taxtable.1.3 <- as.matrix(as.data.frame(rarefied.chloroflexi.clean.int.exclude.1.3@tax_table)) 

rarefied.chloroflexi.int.wide.exclude.md.1.3 <- as.matrix(as.data.frame(rarefied.chloroflexi.clean.int.exclude.1.3@sam_data))

#not same 
identical(chloroflexi.int.wide.exclude.1.3,rarefied.chloroflexi.int.wide.exclude.1.3)

## check if colnames are the same
setequal(rownames(chloroflexi.int.wide.exclude.1.3),rownames(rarefied.chloroflexi.int.wide.exclude.1.3))

### 2 Cyanobacteria subset #####
rarefied.cyanobacteria.clean.int.exclude.1.3 <- new.rarefied.min.int.exclude.1.3 %>%
  subset_taxa(Phylum == "Cyanobacteria" & Class  == "Cyanobacteriia")

rarefied.cyanobacteria.int.wide.exclude.1.3 <- as.matrix(as.data.frame(rarefied.cyanobacteria.clean.int.exclude.1.3@otu_table))

rarefied.cyanobacteria.int.wide.exclude.taxtable.1.3 <- as.matrix(as.data.frame(rarefied.cyanobacteria.clean.int.exclude.1.3@tax_table)) 

rarefied.cyanobacteria.int.wide.exclude.md.1.3 <- as.matrix(as.data.frame(rarefied.cyanobacteria.clean.int.exclude.1.3@sam_data))

#not same 
identical(cyanobacteria.int.wide.exclude.1.3,rarefied.cyanobacteria.int.wide.exclude.1.3)

## check if colnames are the same
setequal(rownames(cyanobacteria.int.wide.exclude.1.3),rownames(rarefied.cyanobacteria.int.wide.exclude.1.3))

## as column names are the same only use rarefied.


### 3 Proteobacteria subset #####
...
rarefied.proteobacteria.clean.int.exclude.1.3 <- new.rarefied.min.int.exclude.1.3 %>%
  subset_taxa(Genus %in% proteobacteria.genus)

rarefied.proteobacteria.int.wide.exclude.1.3 <- as.matrix(as.data.frame(rarefied.proteobacteria.clean.int.exclude.1.3@otu_table))

rarefied.proteobacteria.int.wide.exclude.taxtable.1.3 <- as.matrix(as.data.frame(rarefied.proteobacteria.clean.int.exclude.1.3@tax_table)) 

rarefied.proteobacteria.int.wide.exclude.md.1.3 <- as.matrix(as.data.frame(rarefied.proteobacteria.clean.int.exclude.1.3@sam_data))

#not same 
identical(proteobacteria.int.wide.exclude.1.3,rarefied.proteobacteria.int.wide.exclude.1.3)

## check if colnames are the same
setequal(rownames(proteobacteria.int.wide.exclude.1.3),rownames(rarefied.proteobacteria.int.wide.exclude.1.3))

## colnames are the same so use unrarefied only.



### 4 Other photosynthetic subset #####
other.photo <- c("Chlorobiales","Chloracidobacteriales")

rarefied.otherphoto.clean.int.exclude.1.3 <- new.rarefied.min.int.exclude.1.3 %>% subset_taxa(Order %in% other.photo)

rarefied.otherphoto.int.wide.exclude.1.3 <- as.matrix(as.data.frame(rarefied.otherphoto.clean.int.exclude.1.3@otu_table))

rarefied.otherphoto.int.wide.exclude.taxtable.1.3 <- as.matrix(as.data.frame(rarefied.otherphoto.clean.int.exclude.1.3@tax_table)) 

rarefied.otherphoto.int.wide.exclude.md.1.3 <- as.matrix(as.data.frame(rarefied.otherphoto.clean.int.exclude.1.3@sam_data))

#not same 
identical(otherphoto.int.wide.exclude.1.3,rarefied.otherphoto.int.wide.exclude.1.3)

## check if colnames are the same
setequal(rownames(otherphoto.int.wide.exclude.1.3),rownames(rarefied.otherphoto.int.wide.exclude.1.3))

## colnames are the same so use unrarefied only

#### all 4 photosynthtic groups combined only for rarefied ####

photosynthetic.class <- c("Cyanobacteriia","Chloroflexia")

rarefied.photosynthetic.clean.int.exclude.1.3 <- new.rarefied.min.int.exclude.1.3 %>%
  subset_taxa( Class %in% photosynthetic.class | Order %in% other.photo | Genus %in% proteobacteria.genus )

rarefied.photosynthetic.int.wide.exclude.1.3 <- as.matrix(as.data.frame(rarefied.photosynthetic.clean.int.exclude.1.3@otu_table))

rarefied.photosynthetic.int.wide.exclude.taxtable.1.3 <- as.matrix(as.data.frame(rarefied.photosynthetic.clean.int.exclude.1.3@tax_table)) 

rarefied.photosynthetic.int.wide.exclude.md.1.3 <- as.matrix(as.data.frame(rarefied.photosynthetic.clean.int.exclude.1.3@sam_data))
...
test <- as.matrix(as.data.frame(rarefied.chemolithotrophs.int.exclude.1.3.rooted@tax_table))
...
test <- as.matrix(as.data.frame(rarefied.only.heterotrophs.int.exclude.1.3.rooted@tax_table))
...
### to add refsequences to phyloseq objects.####

refseqs <- refseq(ps.clean.int.exclude.1.3)

chloroflexi.clean.int.exclude.1.3 <- merge_phyloseq(chloroflexi.clean.int.exclude.1.3,refseqs)

chloroflexi.clean.int.exclude.taxtable.1.3 <- (as.matrix(as.data.frame(chloroflexi.clean.int.exclude.1.3@refseq)))

setequal(rownames(chloroflexi.clean.int.exclude.taxtable.1.3),colnames(chloroflexi.int.wide.exclude.1.3))
...
### Create another workspace to use for the next set of codes for phyloseq filtering and processing

### Remove all unwanted objects from the loaded workspace such that only the phyloseq objects are present.

### Either remove objects individually using : e.g 'rm(fnFs)'

### Or using a pattern : e.g  'rm(list=ls(pattern="seqtab.nochim"))'
